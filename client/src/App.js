...
function App() {
  ...
  useEffect(() => {
    ...
    if (selected && gtMap) {
      gtMap.innerHTML = '';
      window.trends.embed.renderExploreWidgetTo(gtMap, "GEO_MAP",
        {"comparisonItem": [{"keyword":selected.key,"geo":"","time":"2004-01-01 2021-05-20"}],
          "category":0,"property":""}, {"exploreQuery": `q=${selected.key}&date=all`,"guestPath":"https://trends.google.com:443/trends/embed/"})
    }
  }, [selected]);
          ...
          <Col span={19}>
            {selected ? <>
              <div>
                ...
                <span style={{float: 'right'}}>{selected.previous.filter(a => !!a).join('-')}</span>
              </div>
              <div>
                <iframe
                  id="google-search"
              ...
              <div style={{width: '100%', height: '47vh', display: 'flex'}}>
