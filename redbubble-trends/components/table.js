...
const Row =
  (rows, checkedList, selected, setSelected) =>
  ({ index, style }) => {
    ...
    return (
      <div
        ...
        className={`row-container ${
          selected && row.key === selected.key ? "selected" : ""
        } ${checkedList[row.key] ? "checked" : ""} `}
      >
        <div className="row" onClick={() => setSelected(row)}>
          <div className="number">
            {row.count === "not-set" ? "N/A" : row.count}
          </div>
          ...
          {row.previous.length && (
            <div>
              <XYPlot height={70} width={70}>
                <VerticalBarSeries
                  data={row.previous.map((key, index) => ({
                    x: index,
                    y: key || 0,
                  }))}
                />
    ...
    <div style={{ height: "100%" }}>
      ...
      <Input.Group compact>
        ...
        <Input
          ...
          placeholder="Min"
          onBlur={(e) => setFilter({ ...filter, min: e.target.value })}
        />
        <Input
          style={{ width: 80, textAlign: "center" }}
          placeholder="Max"
          onBlur={(e) => setFilter({ ...filter, max: e.target.value })}
        />
      </Input.Group>
      <Input
        placeholder="search..."
